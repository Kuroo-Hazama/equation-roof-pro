...
const FallbackUI = ({ autoReloading }: { autoReloading: boolean }) => (
  <div
    style={{
      minHeight: "100vh",
      display: "flex",
...
export const RouteErrorBoundary = () => {
  ...
  useEffect(() => {
    ...
    if (!reloading) {
      void logFrontendError(error, { source: "RouteErrorBoundary" });
    }
  }, [error]);

  // Preserve 404 behavior for unknown routes — let NotFound page handle them
  if (isRouteErrorResponse(error) && error.status === 404) {
    return <FallbackUI autoReloading={false} />;
  }

  return <FallbackUI autoReloading={isChunkLoadError(error)} />;
};
