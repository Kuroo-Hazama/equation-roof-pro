          ...
          {categories.map((c) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-subtitle font-medium transition-all ${
                filter === c ? "bg-navy text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"
              }`}
            >
              {c}
            </button>
            ...
            <ScrollReveal key={`${p.title}-${i}`} delay={i * 80}>
              <div className="card-equation overflow-hidden">
                <div className="relative">
                  ...
                  <span className="absolute top-3 left-3 bg-gold text-secondary-foreground text-xs font-subtitle font-semibold px-3 py-1 rounded-full">
                    {p.cat}
                  </span>
                </div>
                <div className="p-5">
                  ...
                  <p className="text-gold font-subtitle font-semibold text-sm mt-2">{p.surface}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
